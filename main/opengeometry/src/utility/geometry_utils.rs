...
fn project_to_frame(work_plane: &WorkPlane,point: &Vector3,) -> (f64, f64) {
    let (dx, dy, dz) = (
...
fn cross_2d(ax: f64, ay: f64, bx: f64, by: f64) -> f64 {ax * by - ay * bx}
...
fn line_intersection_2d(a: (f64, f64),da: (f64, f64),b: (f64, f64),db: (f64, f64),) -> Option<(f64, f64)> {
...
fn ccw_angle(vector: &Vector3, work_plane: &WorkPlane) -> f64 {
